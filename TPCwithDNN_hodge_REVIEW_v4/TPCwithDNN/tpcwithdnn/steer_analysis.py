...
# OLD| tf.random.set_seed(SEED)
try:  # [BARU] TF opsional: jalur struktural memakai PyTorch
...
# OLD| # To suppress ROOT-specific command line help
# OLD| from ROOT import PyConfig # pylint: disable=import-error
# OLD| PyConfig.IgnoreCommandLineOptions = True
# [BARU] ROOT opsional (hanya untuk jalur validasi asli)
try:
...
# OLD| import tpcwithdnn.check_root # pylint: disable=unused-import
try:  # [BARU] check_root memanggil sys.exit(1) tanpa ROOT -> dijinakkan
...
# OLD| from tpcwithdnn.idc_data_validator import IDCDataValidator
try:  # [BARU] validator butuh ROOT; opsional di jalur struktural
    from tpcwithdnn.idc_data_validator import IDCDataValidator
except (ImportError, SystemExit):
    IDCDataValidator = None

def setup_tf():
    """
    Limit GPU memory usage.
    """
# OLD|     if os.environ.get("TPCwithDNNSETMEMLIMIT"):
    if tf is None:  # [BARU] tanpa TF tidak ada yang perlu diatur
        return
...
def init_models(config_parameters):
    ...
    if config_parameters["corr"]["active"] is True:
        corr = None
# OLD|     dataval = IDCDataValidator()
    dataval = IDCDataValidator() if IDCDataValidator is not None else None  # [BARU]
...
def run_model_and_val(model, dataval, default, config_parameters):
    ...
    :param dict config_parameters: dictionary of values read from the config file
    """
# OLD|     dataval.set_model(model)
    if dataval is not None:  # [BARU] tanpa ROOT, validator dilewati
