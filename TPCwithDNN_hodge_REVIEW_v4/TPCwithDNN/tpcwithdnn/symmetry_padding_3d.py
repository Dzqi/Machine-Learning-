...
Kode ASLI = baris awal file s.d. penanda KODE-BARU (64 baris, tiap baris berawalan "# OLD| ").
KODE BARU dimulai setelah penanda.
"""

# OLD| """
# OLD| Custom symmetric cube padding.
# OLD| """
# OLD| import tensorflow as tf
# OLD| from keras.layers import Layer
# OLD| 
# OLD| class SymmetryPadding3d(Layer):
# OLD|     """
# OLD|     Custom Keras layer implementing symmetric padding in 3D.
# OLD|     """
# OLD|     def __init__(self, padding=None,
# OLD|                  mode='SYMMETRIC', data_format="channels_last", **kwargs):
# OLD|         """
# OLD|         Initialize the layer.
# OLD| 
# OLD|         :param list padding: list of three 2-element lists describing how many values should be
# OLD|                              inserted before and after tensor contents in each dimension.
# OLD|                              See Tensorflow documentation for a more detailed description.
# OLD|                              Default: unit padding in each dimension.
# OLD|         :param str mode: padding mode, chosen from Tensorflow, default: SYMMETRIC
# OLD|         :param str data_format: what is the dimension order, default: channels_last, i.e.,
# OLD|                                 shape (batch_size, height, width, channels)
# OLD|         """
# OLD|         self.data_format = data_format
# OLD|         self.padding = [[1, 1], [1, 1], [1, 1]] if padding is None else padding
# OLD|         self.mode = mode
# OLD|         super().__init__(**kwargs)
# OLD|         self.output_dim = None
# OLD| 
# OLD|     # pylint: disable=arguments-differ
# OLD|     def call(self, inputs):
# OLD|         """
# OLD|         Defines computations made by the layer during the forward pass through the network.
# OLD| 
# OLD|         :param tf.Tensor inputs: input Tensorflow tensor from the previous layer.
# OLD|         :return: padded input tensor
# OLD|         :rtype: tf.Tensor
# OLD|         """
# OLD|         pad = [[0, 0]] + self.padding + [[0, 0]]
# OLD|         paddings = tf.constant(pad)
# OLD|         out = tf.pad(inputs, paddings, self.mode)
# OLD|         self.output_dim = [(out.shape[0], out.shape[1], out.shape[2], out.shape[3], out.shape[4])]
# OLD|         return out
# OLD| 
# OLD|     def compute_output_shape(self):
# OLD|         """
# OLD|         Overriden function from the base class to get the output shape.
# OLD| 
# OLD|         :return: shape of the output data from the layer
# OLD|         :rtype: list
# OLD|         """
# OLD|         return self.output_dim
# OLD| 
# OLD|     def get_config(self):
# OLD|         """
# OLD|         Overriden base function for obtaining layer settings, used, e.g.,
# OLD|         for saving the layer to JSON.
# OLD| 
# OLD|         :return: layer parameters
# OLD|         :rtype: dict
# OLD|         """
# OLD|         config = {'padding': self.padding, 'data_format': self.data_format, 'mode': self.mode}
# OLD|         base_config = super().get_config()
# OLD|         return dict(list(base_config.items()) + list(config.items()))
