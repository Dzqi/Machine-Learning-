...
class DNNSettings:
    ...
    def __init__(self, common_settings, data_param):
        ...
        # --- [BARU] Parameter jalur struktural TDL+PINN (HodgeNet, v3) ---
        # CATATAN: kunci `epochs` ASLI (dibaca di atas, baris ~176) kini
        # dipakai jalur struktural sebagai loop EPOCH SEJATI.
        self.synthetic = data_param.get("synthetic", False)
        self.synthetic_events = data_param.get("synthetic_events", 2)
        self.synthetic_mode = data_param.get("synthetic_mode", "mc")
        self.mc_pileup = data_param.get("mc_pileup", 3000)
        self.mc_mult_rel_sigma = data_param.get("mc_mult_rel_sigma", 0.3)
        self.mc_charge_rel_sigma = data_param.get("mc_charge_rel_sigma", 0.5)
        self.hodge_hidden = data_param.get("hodge_hidden", 32)
        self.hodge_nblocks = data_param.get("hodge_nblocks", 4)
        self.lam_data = data_param.get("lam_data", 1.0)
        self.lam_pde = data_param.get("lam_pde", 0.1)
        self.residual_mode = data_param.get("residual_mode", "structural_hodge")
        self.omega_tau = data_param.get("omega_tau", 0.32)   # PLACEHOLDER
        self.e0_vcm = data_param.get("e0_vcm", 400.0)        # PLACEHOLDER
        self.make_plots = data_param.get("make_plots", True)
        self.save_root = data_param.get("save_root", True)
        self.network = data_param.get("network", "hodge_unet")
        self.unet_levels = data_param.get("unet_levels", 3)
        self.unet_blocks = data_param.get("unet_blocks", 1)
        self.unet_channel_growth = data_param.get("unet_channel_growth", 2)
